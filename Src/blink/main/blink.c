...
#include <math.h> // sine wave generation
...
static void i2s_example_write_sine_task(void *args)
{
    ...
    int samples = EXAMPLE_BUFF_SIZE / 4; // stereo frames

    for (int i = 0; i < samples; i++) {
        int16_t sample = (int16_t)(AMPLITUDE * 32767 * sinf(phase));

        w_buf[i * 2]     = sample; // Left
        w_buf[i * 2 + 1] = sample; // Right
